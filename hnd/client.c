...
static OSErr _SendMessage (
	...
	if (szpString)
		...
		/* Break the long string into a list
...
static void _UpdateClient (
	...
	/* We only CHECK for an update every interval secs. */
	tUpdate = tCurrent + (StringUpdated (conn->lastUpdate) ? 0 : conn->interval) ;
	...
	if (tCurrent >= tUpdate)
		...
		result = GetString (szBuffer, sizeof (szBuffer)) ;
...
ClientPtr FindClient (
	...
	/* We do not multiplex (kWhoType can't). */
	return pTemp ;
	}
