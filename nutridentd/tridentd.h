...
extern int GetUptimeString (char *, long) ;
extern int GetWhoString (char *, long) ;
extern Boolean WhoUpdated (time_t) ;

/* Functions in ppcstuff.c. */
#if SIMPLIFIED
#define CloseClient(cp) DeleteClient (cp)
#define InitPPCStuff() noErr
#define PPCTimeSlice() noErr
#define PPCShutDown()
#else
extern OSErr CloseClient (ClientPtr) ;
extern OSErr InitPPCStuff (void) ;
extern OSErr PPCTimeSlice (void) ;
extern OSErr PPCShutDown (void) ;
#endif	/* SIMPLIFIED */
